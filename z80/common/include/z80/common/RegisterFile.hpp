#pragma once

#ifndef SPECBOLT_MODULES
#include <array>
#include <cstdint>
#include <iosfwd>
#include <string_view>

#include "z80/common/Flags.hpp"
#endif

namespace specbolt {

SPECBOLT_EXPORT
class RegisterFile {
public:
  enum class R8 { A, F, B, C, D, E, H, L, A_, F_, B_, C_, D_, E_, H_, L_, SPH, SPL, IXH, IXL, IYH, IYL };
  enum class R16 { AF, BC, DE, HL, AF_, BC_, DE_, HL_, SP, IX, IY };

  // Paired registers.
  [[nodiscard]] std::uint8_t get(R8 reg) const;
  void set(R8 reg, std::uint8_t value);
  [[nodiscard]] std::uint16_t get(R16 reg) const;
  void set(R16 reg, std::uint16_t value);

  // Convenience named registers.
  [[nodiscard]] std::uint16_t ix() const;
  [[nodiscard]] std::uint16_t iy() const;
  [[nodiscard]] std::uint16_t sp() const;
  void sp(std::uint16_t sp);

  // Special registers.
  [[nodiscard]] std::uint16_t pc() const;
  void pc(std::uint16_t pc);
  [[nodiscard]] std::uint8_t r() const;
  void r(std::uint8_t r);
  [[nodiscard]] std::uint8_t i() const;
  void i(std::uint8_t i);

  [[nodiscard]] std::uint16_t wz() const;
  void wz(std::uint16_t wz);

  void exx();
  void ex(R16 lhs, R16 rhs);

  void dump(std::ostream &to, std::string_view prefix) const;

private:
  class RegPair {
  public:
    [[nodiscard]] std::uint8_t high() const;
    void high(std::uint8_t high);
    [[nodiscard]] std::uint8_t low() const;
    void low(std::uint8_t low);
    [[nodiscard]] std::uint16_t highlow() const;
    void highlow(std::uint16_t highlow);

  private:
    std::uint8_t low_{0xff};
    std::uint8_t high_{0xff};
  };
  [[nodiscard]] RegPair &reg_for(R8 r8);
  [[nodiscard]] const RegPair &reg_for(R8 r8) const;
  [[nodiscard]] RegPair &reg_for(R16 r16);
  [[nodiscard]] const RegPair &reg_for(R16 r16) const;

  std::array<RegPair, 11> regs_{};
  std::uint16_t wz_{0xffff};
  std::uint16_t pc_{};
  std::uint8_t r_{};
  std::uint8_t i_{};
};

namespace {
bool is_high(RegisterFile::R8 reg) { return (static_cast<unsigned>(reg) & 1) == 0; }
} // namespace

inline std::uint8_t RegisterFile::get(const R8 reg) const {
  const auto &reg_pair = reg_for(reg);
  return is_high(reg) ? reg_pair.high() : reg_pair.low();
}
inline void RegisterFile::set(const R8 reg, const std::uint8_t value) {
  auto &reg_pair = reg_for(reg);
  if (is_high(reg))
    reg_pair.high(value);
  else
    reg_pair.low(value);
}
inline std::uint16_t RegisterFile::get(const R16 reg) const { return reg_for(reg).highlow(); }
inline void RegisterFile::set(const R16 reg, const std::uint16_t value) { reg_for(reg).highlow(value); }
inline std::uint16_t RegisterFile::ix() const { return reg_for(R16::IX).highlow(); }
inline std::uint16_t RegisterFile::iy() const { return reg_for(R16::IY).highlow(); }
inline std::uint16_t RegisterFile::sp() const { return reg_for(R16::SP).highlow(); }
inline void RegisterFile::sp(const std::uint16_t sp) { reg_for(R16::SP).highlow(sp); }
inline std::uint16_t RegisterFile::pc() const { return pc_; }
inline void RegisterFile::pc(const std::uint16_t pc) { pc_ = pc; }
inline std::uint8_t RegisterFile::r() const { return r_; }
inline void RegisterFile::r(const std::uint8_t r) { r_ = r; }
inline std::uint8_t RegisterFile::i() const { return i_; }
inline void RegisterFile::i(const std::uint8_t i) { i_ = i; }
inline std::uint16_t RegisterFile::wz() const { return wz_; }
inline void RegisterFile::wz(const std::uint16_t wz) { wz_ = wz; }

inline void RegisterFile::ex(const R16 lhs, const R16 rhs) { std::swap(reg_for(lhs), reg_for(rhs)); }

inline void RegisterFile::dump(std::ostream &to, std::string_view prefix) const {
  std::print(to, "{}PC: {:04x} | SP: {:04x}\n", prefix, pc(), sp());
  std::print(to, "{}IX: {:04x} | IY: {:04x}\n", prefix, ix(), iy());
  std::print(to, "{}AF: {:04x} - {} | AF': {:04x} - {}\n", prefix, get(R16::AF), Flags(get(R8::F)).to_string(),
      get(R16::AF_), Flags(get(R8::F_)).to_string());
  std::print(to, "{}BC: {:04x} | BC': {:04x}\n", prefix, get(R16::BC), get(R16::BC_));
  std::print(to, "{}DE: {:04x} | DE': {:04x}\n", prefix, get(R16::DE), get(R16::DE_));
  std::print(to, "{}HL: {:04x} | HL': {:04x}\n", prefix, get(R16::HL), get(R16::HL_));
}

inline void RegisterFile::exx() {
  ex(R16::BC, R16::BC_);
  ex(R16::DE, R16::DE_);
  ex(R16::HL, R16::HL_);
}
inline std::uint8_t RegisterFile::RegPair::high() const { return high_; }
inline void RegisterFile::RegPair::high(const std::uint8_t high) { high_ = high; }
inline std::uint8_t RegisterFile::RegPair::low() const { return low_; }
inline void RegisterFile::RegPair::low(const std::uint8_t low) { low_ = low; }
inline std::uint16_t RegisterFile::RegPair::highlow() const { return static_cast<std::uint16_t>(high_ << 8u) | low_; }
inline void RegisterFile::RegPair::highlow(const std::uint16_t highlow) {
  high_ = highlow >> 8 & 0xff;
  low_ = highlow & 0xff;
}

// The relationship between the various register numbers we rely on below is tested here explicit;y.
static_assert(static_cast<unsigned>(RegisterFile::R8::A) == static_cast<unsigned>(RegisterFile::R16::AF) << 1);
static_assert(static_cast<unsigned>(RegisterFile::R8::F) == (static_cast<unsigned>(RegisterFile::R16::AF) << 1) + 1);
static_assert(static_cast<unsigned>(RegisterFile::R8::A) + 8 == static_cast<unsigned>(RegisterFile::R8::A_));
static_assert(static_cast<unsigned>(RegisterFile::R16::AF) + 4 == static_cast<unsigned>(RegisterFile::R16::AF_));

inline RegisterFile::RegPair &RegisterFile::reg_for(R8 r8) { return regs_[static_cast<unsigned>(r8) / 2u]; }
inline const RegisterFile::RegPair &RegisterFile::reg_for(R8 r8) const { return regs_[static_cast<unsigned>(r8) / 2u]; }
inline RegisterFile::RegPair &RegisterFile::reg_for(R16 r16) { return regs_[static_cast<unsigned>(r16)]; }
inline const RegisterFile::RegPair &RegisterFile::reg_for(R16 r16) const { return regs_[static_cast<unsigned>(r16)]; }

} // namespace specbolt
