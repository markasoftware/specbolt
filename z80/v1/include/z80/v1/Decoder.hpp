...
__attribute__((always_inline)) inline static Instruction add_4(Instruction i) {
  i.decode_t_states += 4;
  return i;
}
...
__attribute__((always_inline)) Instruction decode_bit(const std::span<const std::uint8_t> opcodes) {
...
template<typename RegisterSet>
__attribute__((always_inline)) Instruction decode_ddfd(const std::span<const std::uint8_t> opcodes) {
...
__attribute__((always_inline)) Instruction decode_ed(const std::span<const std::uint8_t> opcodes) {
  switch (const auto opcode = opcodes[0]) {
    ...
    case 0x4f: return {"ld {}, {}", 2, Op::LoadSpecial, Operand::R, Operand::A};
    case 0x5f: return {"ld {}, {}", 2, Op::LoadSpecial, Operand::A, Operand::R};
